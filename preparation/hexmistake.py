        ...
        if filename.endswith(".png"):  # Check if the file is an image, adjust as needed for other formats
            ...
            for i in range(image.width):
                for j in range(image.height):
                    ...
                    if pixels[i, j][:3] == '2f13c7':
                        pixels[i, j] = (29, 25, 181)

            # Save the modified image
            image.save('version/training/hello/{0}'.format(filename))  # Overwrite the original image
            # Or save as a new file:
            # image.save(os.path.join(directory, f"modified_{filename}"))

# Example usage
directory_path = 'version/training/hello'
hex_colors = ['00ff00', 'ff0000', '0000ff', '1d19b5', 'ffff00', 'ff00ff', '00ffff', '3300cc'
